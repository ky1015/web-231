...
// Global variables
// added "" to the cash, bill, change ids
...
// Code Event handlers to be run when the cash or bill value changes
// changed incorrect "runRegister" to "runTheRegister"
...
function runTheRegister() {
  ...
  let changeValue = cashBox.value - billBox.value; // calculate the
  let message = document.getElementById("warning");
  message.innerHTML = "";

  //add try catch for when bill is higher than amount in register
  try {
    if (!(changeValue >= 0)) throw "Cash amount doesn't cover the bill";
    changeBox.value = formatCurrency(changeValue); // formats the change as currency
